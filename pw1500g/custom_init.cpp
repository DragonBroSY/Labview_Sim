...
custom_init::custom_init() {
	first_pass = 1;
	std::cout << "From constructor--first_pass is:" << first_pass << "\n";
}
...
float* custom_init::comm_init() {
	...
	std::cout << "shmid from SHEM is: " << shmid_master << "\n";
...
float* custom_init::update(float* data) {
	...
	std::cout << "first_pass is:" << first_pass << "\n";
	...
	std::cout << "shmptr val BEFORE: " << shmptr[2] << " " << shmptr[3] << " " << shmptr[4] << " " << shmptr[5] << " " << shmptr[6]<< '\n';
	...
	std::cout << "shmptr val AFTER: " << shmptr[2] << " " << shmptr[3] << " " << shmptr[4] << " " << shmptr[5] << " " << shmptr[6] << '\n';
...
float* custom_init::send2server() {
	if (custom_init::first_pass == 1) {
		custom_init::shmptr = custom_init::comm_init();
		custom_init::first_pass = 0;
	}
	custom_init::shmptr = getShmptr();
	std::cout << "custom_init::send2server() is called\n";
	std::cout << "first_pass is:" << first_pass << "\n";
	std::cout << "key.c is: " << key.c << " key.t is: " << key.t << "\n";
	std::cout << "shmid is: " << getShmid() << "\n";
	/* wait 50 us */
	...
	std::cout << "shmptr inside send2server() is:" << custom_init::shmptr << "\n";
	std::cout << "shmptr val BEFORE: " << shmptr[0] << " " << shmptr[1] << '\n';

	custom_init::data = shmptr;

	std::cout << "shmptr val AFTER: " << custom_init::data[0] << " " << custom_init::data[1] << '\n';

	std::cout << "Shem writing complete..\n";
	std::cout << "shmptr INSIDE send2server() is:" << shmptr << "\n";
	/* wait 50 us */
	usleep(5000);
	return shmptr;
}