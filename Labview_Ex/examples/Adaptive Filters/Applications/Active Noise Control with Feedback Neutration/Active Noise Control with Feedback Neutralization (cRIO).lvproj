...
&lt;/ExampleProgram&gt;</Property>
	<Property Name="NI.Project.Description" Type="Str">This example demonstrates how to apply a filtered-x LMS adaptive filter to active noise control (ANC) with acoustic feedback neutralization applications by using NI CompactRIO hardware. This example implements the filtered-x LMS algorithm with the LabVIEW FPGA Module, uses the NI 9234 module to acquire the primary noise for reference microphone and error signals for error microphone, uses the NI 9263 module to generate a control waveform for the control speaker. Refer to an illustration of this example, available by opening this example and double-clicking the Active Noise Control with Feedback Neutralization (RT) VI in the &lt;b&gt;RT CompactRIO Target&lt;/b&gt; folder, for information about the setup of this application.

The control waveform radiates upstream to the reference microphone, which results in a corrupted primary noise. ANC with acoustic feedback neutralization can cancel this effect. This example uses the NI 9234 module in slot 1 and the NI 9263 module in slot 2 of a cRIO-9033 backplane. 
...
1. Prepare necessary hardware. You need an NI CompactRIO chassis, a controller, NI 9234 module, NI 9263 module, two microphones, a speaker, and necessary accessories such as cables. If you are setting up a demo system, you can use another speaker to play noise that simulates the primary noise you want to control. You can use the Noise Generation VI included in this example to generate simulated primary noise.

2. Set up the system. This example assumes you use a speaker to simulate the primary noise. Place the control speaker opposite and close to the speaker that generates the primary noise. Distance between the two speakers must be as small as several centimeters. Place a microphone between the two speakers. This microphone is the error microphone. Place the other microphone close to the speaker that generates the primary noise. This microphone is the reference microphone. Plug the NI 9234 module to slot 1 and the NI 9263 module to slot 2 of the CompactRIO chassis. Connect the error microphone to the AI0 channel of the NI 9234 module. Connect the reference microphone to the AI1 channel of the NI 9234 module. Connect the amplifier of the control speaker to the AO0 channel of the NI 9263 module.
