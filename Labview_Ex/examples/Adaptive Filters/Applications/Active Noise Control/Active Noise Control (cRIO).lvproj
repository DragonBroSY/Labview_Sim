...
&lt;/ExampleProgram&gt;</Property>
	<Property Name="NI.Project.Description" Type="Str">This example demonstrates how to apply a filtered-x LMS adaptive filter to active noise control applications by using NI CompactRIO hardware. This example was created and precompiled by using an NI 9234 module in slot 1 and an NI 9263 module in slot 2 of a cRIO-9033 backplane. For more information about moving this example to another FPGA target, visit ni.com/info and enter the info code fpgaex. 

This example implements the filtered-x LMS algorithm with LabVIEW FPGA, uses the NI 9234 module to acquire the primary noise and error signals, uses the NI 9263 module to generate a control waveform for the control speaker. You can use this example for monotone noise control only and you need to know the frequency of the monotone noise in advance.

Complete the following steps to use this example.

1. Prepare necessary hardware. You need an NI CompactRIO chassis, controller, NI 9234 module, NI 9263 module, a microphone, a speaker, and neccessary accessories such as cables. This example assumes you know the frequency of the primary noise, so you do not need another microphone to acquire the primary noise. If you are setting up a demo system, you might use another speaker to play a monotone noise that simulates the primary noise you want to control. You can use the Noise Generation VI in this project to play a simulated primary noise.

2. Set up the system. This example assumes you use a speaker to simulate the primary noise. Place the control speaker opposite and close to the speaker that plays the primary noise. The distance between two speakers must be as small as several centimeters. Place a microphone between the two speakers. This microphone is the error microphone. Plug the NI 9234 module to slot 1 and the NI 9263 module to slot 2 of the CompactRIO chassis. Connect the microphone to the AI0 channel of the NI 9234 module. Connect the amplifier of the control speaker to the AO0 channel of the NI 9263 module.
